...
export class TwitterPostClient {
    ...
    stopProcessingActions: boolean = false;
    isDryRun: boolean;
    bot: Telegraf<Context>;
    messageManager: MessageManager;
    pendingTweets: Map<string, { content: string, roomId: string, newTweetContent: string }> = new Map();
    ...
    constructor(client: ClientBase, runtime: IAgentRuntime) {
        ...
        if (!botToken) {
            elizaLogger.warn("No Telegram bot token provided");
        } else {
    ...
    private async initializeBot(): Promise<void> {
        if (!this.bot) {
            elizaLogger.warn("No Telegram bot instance available to initialize");
            return;
        }
    ...
    private setupMessageHandlers(): void {
        ...
        this.bot.command("approve", async (ctx) => {
            try {
                ...
                if (!replyToMessage) {
                    await ctx.reply("Please reply to a pending tweet with /approve");
                    return;
                }

                const messageId = replyToMessage.message_id.toString();
                const pendingTweet = this.pendingTweets.get(messageId);

                if (!pendingTweet) {
                    await ctx.reply("No pending tweet found for this message");
                    return;
                }

                const { content, roomId, newTweetContent } = pendingTweet;
                this.postTweet(
                ...
                await ctx.reply("Tweet has been posted! ✅");
            } catch (error) {
                elizaLogger.error("Error handling approve command:", error);
                await ctx.reply("Failed to post the tweet. Please try again.");
            }
        });
        ...
        this.bot.command("edit", async (ctx) => {
            try {
                ...
                const replyToMessage = ctx.message.reply_to_message;
                if (!replyToMessage) {
                    await ctx.reply("Please reply to a pending tweet with /edit YOUR_EDITED_TWEET");
                    return;
                }
                ...
                // Get the edited content after the /edit command
                const editedContent = ctx.message.text.split("/edit")[1]?.trim();
                if (!editedContent) {
                    await ctx.reply("Please provide the edited tweet content after /edit");
                    return;
                }

                // Update the pending tweet with new content
                this.pendingTweets.set(messageId, {
                    ...pendingTweet,
                    content: editedContent
                });

                await ctx.reply(`Tweet updated! Preview:\n\n${editedContent}\n\nUse /approve to post or /edit to make more changes.`);
            } catch (error) {
                elizaLogger.error("Error handling edit command:", error);
                await ctx.reply("Failed to edit the tweet. Please try again.");
            }
        });
    ...
    async postTweet(
        ...
        try {
            ...
            // Send tweet to Telegram if bot is initialized
            if (this.messageManager && this.bot) {
                try {
                    const tweetUrl = tweet.permanentUrl;
                    const telegramMessage = `New tweet from @${twitterUsername}:\n\n${cleanedContent}\n\nView on Twitter: ${tweetUrl}`;

                    await this.messageManager.sendMessageInChunks(
                        { chat: { id: process.env.TELEGRAM_CHAT_ID } } as Context,
                        { text: telegramMessage }
                    );

                    elizaLogger.log("Tweet sent to Telegram successfully");
                } catch (error) {
                    elizaLogger.error("Error sending tweet to Telegram:", error);
                }
            }
        } catch (error) {
    ...
    private async generateNewTweet() {
        ...
        try {
            ...
            if (this.isDryRun) {
                ...
                // can test senting tg message from dry run too
            ...
            try {
                elizaLogger.log(`Send to Telegram for approval:\n ${cleanedContent}`);

                // Send to Telegram for approval
                if (this.bot) {
                    const previewMessage = `📝 New tweet ready for review:\n\n${cleanedContent}\n\nUse:\n/approve - Post the tweet\n/edit - Edit the tweet content`;
                    ...
                    // Store the pending tweet info
                    if (sentMessage && sentMessage[0]) {
                        this.pendingTweets.set(sentMessage[0].message_id.toString(), {
                    ...
                    elizaLogger.log("Tweet preview sent to Telegram for approval");
                }
            } catch (error) {
                    elizaLogger.error("Error sending tweet preview to Telegram:", error);
            }
        } catch (error) {
    ...
    /**
     * Generates twe`et content based on the provided state and template.
     */
